...
export default function VideoMockInterviewPage() {
  ...
  const handleStartInterview = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/interview/start`, {
        ...
        body: JSON.stringify({ jobRole, interviewType: 'Technical', experience, category, difficulty })
      });
  ...
  const handleRecordingComplete = async (audioBlob: Blob, sampledFrames: string[], metricsList: BodyLanguageMetrics[]) => {
    if (!session) return;
    setView('loading');
    ...
    try {
      ...
      if (!audioRes.ok) throw new Error(audioData.error || 'Failed to transcribe audio');

      const transcribedText = audioData.transcribedText || "No speech detected.";
      ...
      if (!submitRes.ok) throw new Error(submitData.error || 'Failed to evaluate answer');

      // 3. Submit Video Frames for Vision Analysis
      if (sampledFrames.length > 0) {
        const visionRes = await fetch(`${API_URL}/interview/analyze-video-frames`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
          body: JSON.stringify({
            sessionId: session._id,
    ...
    <div className="h-full flex flex-col max-w-6xl mx-auto px-4 py-8">
      {view === 'setup' && (
        <div className="flex-1 flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-500">
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight mb-4">Video Mock Interview</h1>
          <p className="text-slate-500 text-center max-w-xl mb-12">
            Practice with an AI interviewer. We'll analyze both what you say and your body language—eye contact, posture, and facial expressions—using real-time vision AI.
          </p>
        ...
        <div className="flex-1 flex flex-col items-center animate-in fade-in slide-in-from-bottom-8 duration-500 w-full mt-4">
          <div className="w-full mb-6 max-w-4xl mx-auto">
            <h2 className="text-xl font-bold text-slate-900">
              Q{currentQuestionIndex + 1}: {session.questions[currentQuestionIndex]?.text}
            </h2>
            <div className="flex gap-1.5 mt-4">
              {session.questions.map((_: any, i: number) => (
                <div key={i} className={`h-1.5 flex-1 rounded-full ${i < currentQuestionIndex ? 'bg-emerald-400' : i === currentQuestionIndex ? 'bg-indigo-500' : 'bg-slate-200'}`} />
              ))}
            </div>
          </div>
      ...
      {view === 'loading' && (
        <div className="flex-1 flex flex-col items-center justify-center animate-in fade-in duration-500">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Analyzing Video & Speech</h2>
          <p className="text-slate-500">Running Vision LLM and Body Language Metrics...</p>
        </div>
      )}

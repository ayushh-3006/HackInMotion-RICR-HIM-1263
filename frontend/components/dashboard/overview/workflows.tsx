...
import { FileText, Mic, UploadCloud, Type, Video, ArrowRight, CheckCircle2, Loader2, X } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col w-full h-full">
          ...
          >
            ...
            {file ? (
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-xs font-medium text-slate-500 mb-3">{(file.size / 1024 / 1024).toFixed(2)} MB</p>
                <button 
          ...
          <div className="relative mb-4 flex-grow">
            ...
            />
            <span className={`absolute bottom-2 right-2 text-[10px] font-bold ${jdText.length > 5000 ? 'text-red-500' : 'text-slate-400'}`}>
              {jdText.length} / 5000
            </span>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col w-full h-full">
          ...
          <ul className="space-y-3 mb-6 flex-grow">
            {getModeFeatures(mode).map((feature, idx) => (
              <li key={idx} className="flex items-center gap-2 text-sm font-medium text-slate-600 animate-in fade-in slide-in-from-left-2 duration-300">

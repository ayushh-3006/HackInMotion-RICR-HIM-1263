...
import { Lightbulb, Info, CheckCircle2, ChevronRight, ChevronDown, AlertTriangle } from 'lucide-react';
...
const suggestions: SuggestionItem[] = [
  {
    ...
    explanation: 'ATS systems and recruiters look for measurable results to validate your skills.',
    before: 'Developed new features for the company website.',
    after: 'Developed 5 new interactive features, increasing user engagement by 25% over 3 months.'
  },
  {
    id: 's2',
    ...
    explanation: 'The target job description heavily emphasizes these skills, but they are missing from your skills section.',
  ...
  },
  {
    ...
    explanation: 'Your current summary is too generic and does not clearly position you as a full-stack engineer.',
    before: 'Software Engineer with experience in web development.',
    after: 'Results-driven Full-Stack Engineer with 3+ years of experience in React and Node.js...'
  }
];
      ...
      <div className="space-y-3">
        {activeItems.length === 0 ? (
          <div className="text-sm text-slate-500 text-center py-4">
            All suggestions applied! You are good to go.
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-3 flex-grow">
                  ...
                  <div className="flex flex-col">
                    <span className={`text-sm font-medium transition-colors ${isExpanded ? 'text-indigo-900' : 'text-slate-700 group-hover:text-slate-900'}`}>
                      {item.title}
                    </span>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] font-bold text-slate-500">{item.category}</span>
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${getPriorityBadge(item.priority)}`}>
                ...
                <div className="flex-shrink-0 ml-4">
                  ...
                  >
                    <ChevronRight className={`w-4 h-4 transition-colors ${isExpanded ? 'text-indigo-500' : 'text-slate-400 group-hover:text-indigo-400'}`} />
                  </motion.div>
                </div>
              </div>
                    ...
                    <div className="px-4 pb-4 pt-1 ml-11 border-t border-slate-100/50 mt-1">
                      ...
                      <div className="bg-white rounded-lg p-3 border border-slate-200 mb-4 shadow-sm text-sm">
                        <div className="flex items-start gap-2 mb-2">
                          <span className="font-bold text-red-500 flex-shrink-0">Before:</span>
                          <span className="text-slate-500 line-through">{item.before}</span>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="font-bold text-emerald-500 flex-shrink-0">After:</span>
                          <span className="text-slate-800 font-medium">{item.after}</span>
                        </div>
                      </div>

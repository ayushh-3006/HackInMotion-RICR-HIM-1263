...
export function VisionFeedbackReport({ session }: VisionFeedbackReportProps) {
  ...
  const avgEyeContact =
    Math.round(
      allAnswers.reduce(
        (sum: number, a: any) => sum + (a.eyeContactScore || 0),
        0,
      ) / allAnswers.length,
    ) || 0;
  const avgPosture =
    Math.round(
      allAnswers.reduce(
        (sum: number, a: any) => sum + (a.postureScore || 0),
        0,
      ) / allAnswers.length,
    ) || 0;
  const avgExpression =
    Math.round(
      allAnswers.reduce(
        (sum: number, a: any) => sum + (a.expressionScore || 0),
        0,
      ) / allAnswers.length,
    ) || 0;

  const overallBodyScore =
    Math.round((avgEyeContact + avgPosture + avgExpression) / 3) || 0;
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        >
          <UserCheck
            className={`w-8 h-8 mb-2 ${getScoreColor(overallBodyScore)}`}
          />
          <span className="text-sm font-bold uppercase tracking-wider text-slate-600">
            Overall Grade
          </span>
          <span
            className={`text-5xl font-black mt-2 ${getScoreColor(overallBodyScore)}`}
          >
            {overallBodyScore}
          </span>
        </motion.div>
      ...
      <div className="space-y-6">
        ...
        {allAnswers.map((ans: any, idx: number) => (
          ...
          >
            <h4 className="font-bold text-lg text-slate-900 border-b border-slate-100 pb-3">
              Q{idx + 1}. Vision Analysis
            </h4>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="text-center p-3 bg-slate-50 rounded-xl">
                ...
                <p
                  className={`text-xl font-black ${getScoreColor(ans.eyeContactScore || 0)}`}
                >
                  {ans.eyeContactScore || 0}
                </p>
              </div>
              <div className="text-center p-3 bg-slate-50 rounded-xl">
                <p className="text-xs font-bold text-slate-400 uppercase">
                ...
                <p
                  className={`text-xl font-black ${getScoreColor(ans.postureScore || 0)}`}
                >
                  {ans.postureScore || 0}
                </p>
              </div>
              <div className="text-center p-3 bg-slate-50 rounded-xl">
                ...
                <p
                  className={`text-xl font-black ${getScoreColor(ans.expressionScore || 0)}`}
                >
                  {ans.expressionScore || 0}
                </p>
              </div>
            </div>
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4">
              ...
              <p className="text-sm text-indigo-700 leading-relaxed">
                {ans.bodyLanguageFeedback ||
                  "No detailed vision feedback provided."}
              </p>
            </div>
            ...
            {ans.flaggedMoments && ans.flaggedMoments.length > 0 && (
              <div className="mt-4">
                <h5 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4 text-amber-500" /> Flagged
                  Moments
                </h5>
                <ul className="space-y-2">
                  {ans.flaggedMoments.map((flag: any, i: number) => (
                    <li
                      key={i}
                      className="text-sm flex items-center gap-2 text-slate-600 bg-amber-50/50 p-2 rounded-lg border border-amber-100"
                    >
                      <span className="font-mono font-bold text-amber-600 bg-amber-100 px-2 py-0.5 rounded text-xs">
                        {Math.floor(flag.timestamp / 60)}:
                        {(flag.timestamp % 60).toString().padStart(2, "0")}
                      </span>
                      {flag.description}
                    </li>
                  ))}
